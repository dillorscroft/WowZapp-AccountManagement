    ...
    public sealed class Device
    {
        ...
        public void Save()
        {
            ...
            try
            {
                ...
                this.dt.ExecuteCommand(cmd, DataTools.DataSources.LOLAccountManagement);
            }
            catch (Exception ex)
            {
                var errorObject = ErrorManagement.CreateErrorObject(SystemTypes.ErrorMessage.DeviceSaveFailed, this.ToString() + ".Save()");
                errorObject.ErrorDescription = ex.Message;                
                this.Errors.Add(errorObject);
                new LOLAccountRepository(this.dt).LogError(new MethodParameters(this.DeviceID, this.DeviceType, this.AccountID).ParametersList, this.ToString() + ".Save()", errorObject);
            }
        }
