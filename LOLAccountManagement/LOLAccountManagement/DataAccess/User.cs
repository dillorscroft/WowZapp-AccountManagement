    ...
    public sealed class User
    {
        ...
        public User(DataRow dr)
        {
            try
            {
                ...
                byte[] pictureData = (byte[])dr["Picture"];
        ...
        public void Get()
        {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (!DBNull.Value.Equals(dr["LastName"])) { this._LastName = (string)dr["LastName"]; }
                    if (!DBNull.Value.Equals(dr["EmailAddress"])) { this._EmailAddress = Cryptography.Decrypt((string)dr["EmailAddress"]); }
                    //removed by AD 16/07/2012 so that no password / image object is returned
                    //if (!DBNull.Value.Equals(dr["Picture"])) { _Picture = ImageHandler.byteArrayToImage((Byte[])dr["Picture"]); }
                    //if (!DBNull.Value.Equals(dr["Password"])) { this._Password = Cryptography.Decrypt((string)dr["Password"]); }
                    if (!DBNull.Value.Equals(dr["AccountActive"])) { this._AccountActive = (bool)dr["AccountActive"]; }
                    if (!DBNull.Value.Equals(dr["DateOfBirth"])) { this._DateOfBirth = (DateTime)dr["DateOfBirth"]; }
            ...
            try
            {
                ...
                else
                {
                    if(Password.Length > 0)
                        ...
                        cmd.CommandText = "UPDATE Users SET FirstName = @FirstNAme, LastName = @LastName, EmailAddress = @EmailAddress, Picture = @Picture, AccountActive = @AccountActive, DateOfBirth = @DateOfBirth, PictureURL = @PictureURL WHERE AccountID = @AccountID;";
                }

                byte[] pictureData = new byte[0];
                if (this.Picture.Length > 0)
                {
                    JpegConversion jgc = new JpegConversion(this.Picture, Guid.NewGuid() + ".jpg", Guid.NewGuid() + ".jpg", "ImageDecoder", "i_view32.exe");
                    pictureData = jgc.RunMainBody(HostingEnvironment.ApplicationPhysicalPath, Directory.GetCurrentDirectory());
                }
