...
{
    public class Test_UserGetResetToken : TestBase, ITestable
    {
        //Test_UserGetResetToken(Guid AccountID, LOLConnect.AccountOAuth.OAuthTypes, string OAuthID, string OAuthType, Guid token)
        ...
        {
            this.Test_UserGetResetToken_AccountIdNotLinkedToToken_ShouldFail();
            this.Test_UserGetResetToken_TokenNotAuthenticated_ShouldFail();
            this.Test_UserGetResetToken_TokenExpired_ShouldFail();
            this.Test_UserGetResetToken_TokenLoggedOut_ShouldFail();
            this.Test_UserGetResetToken_TokenNotInDatabase_ShouldFail();
            this.Test_UserGetResetToken_ValidInput_ShouldSucceed();

        }
        #endregion

        #region Constructor
        public Test_UserGetResetToken(LOLConnect.LOLConnectClient ws, ILogger logger)
            : base()
        {
            this._ws = ws;
            this.Logger = logger;
        ...
        private void Test_UserGetResetToken_TokenNotAuthenticated_ShouldFail()
        {
            this.Logger.LogMessage("Testing Test_UserGetResetToken_TokenNotAuthenticated_ShouldFail ...", true);
            ...
            LOLConnect.GeneralError genError = _ws.UserGetResetToken(newEmail, token);
            elapsed.Stop();
        ...
        private void Test_UserGetResetToken_TokenExpired_ShouldFail()
        {
            this.Logger.LogMessage("Testing Test_UserGetResetToken_TokenExpired_ShouldFail ...", true);
            this.Logger.LogMessage("Not Implemented Yet ...", true);
            this.Logger.LogMessage(this.Delimiter, true);
            this.CleanAfterTest(this._ws);
        }

        private void Test_UserGetResetToken_TokenLoggedOut_ShouldFail()
        {
            this.Logger.LogMessage("Testing Test_UserGetResetToken_TokenLoggedOut_ShouldFail ...", true);
            Image profileImage = Image.FromFile(this.ImageFilePath);
            Guid token = _ws.AuthenticationTokenGet(this.RandomDeviceID);
            string newEmail = this.GetRandomEmail();
            LOLConnect.User tmpUser = _ws.UserCreate(this.RandomDeviceID, LOLConnect.DeviceDeviceTypes.Windows, LOLConnect.AccountOAuth.OAuthTypes.LOL, string.Empty, string.Empty, this.RandomFirstName, this.RandomLastName, newEmail, this.RandomPassword, ImageHandler.imageToByteArray(profileImage), new DateTime(1978, 2, 5), token);
        ...
        private void Test_UserGetResetToken_TokenNotInDatabase_ShouldFail()
        {
            this.Logger.LogMessage("Testing Test_UserGetResetToken_TokenNotInDatabase_ShouldFail ...", true);

            var elapsed = Stopwatch.StartNew();
            LOLConnect.GeneralError genError = _ws.UserGetResetToken(this.GetRandomEmail() , Guid.NewGuid());
            elapsed.Stop();
            this.Logger.LogMessage(PrepareElapsedTimeOutput(elapsed), true);
        ...
        private void Test_UserGetResetToken_AccountIdNotLinkedToToken_ShouldFail()
        {
            this.Logger.LogMessage("Testing Test_UserGetResetToken_AccountIdNotLinkedToToken_ShouldFail ...", true);
            Guid token = _ws.AuthenticationTokenGet(this.RandomDeviceID);
            ...
            if (!string.IsNullOrEmpty(genError.ErrorDescription) && genError.ErrorDescription.Equals(SystemTypes.ErrorMessage.AuthenticationTokenDoesNotMatchAccountID.ToString()))
                this.Logger.LogMessage(this.TestSuccessMessage, true);
            else
                this.Logger.LogMessage(this.TestFailMessage, true);
        ...
        private void Test_UserGetResetToken_ValidInput_ShouldSucceed()
        {
            this.Logger.LogMessage("Testing Test_UserGetResetToken_ValidInput_ShouldSucceed ...", true);
