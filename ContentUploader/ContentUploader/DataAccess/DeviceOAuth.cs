namespace ContentUploader
{
    ...
    public sealed class DeviceOAuth
    {
        ...
        /// <summary>
        /// Initializes a new instance of the DeviceOAuth class
        /// </summary>
        /// <param name="residentID">Unique Database ID</param>
        public DeviceOAuth(string deviceID, OAuthTypes oAuthType)
        {
        ...
        public void Get()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT * FROM DeviceOAuth WHERE DeviceID = @DeviceID AND OAuthTypeID = @OAuthType";
        ...
        public void Save()
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = "SELECT DeviceID FROM DeviceOAuth WHERE DeviceID = @DeviceID AND OAuthTypeID = @OAuthType";
                cmd.Parameters.Add("@DeviceID", SqlDbType.NVarChar, 100).Value = _DeviceID;
                cmd.Parameters.Add("@OAuthType", SqlDbType.Int).Value = (int)_OAuthType;
                DataSet ds = dt.GetDataSet(cmd, DataTools.DataSources.LOLAccountManagement);
                if (ds.Tables[0].Rows.Count == 0)
                {
                    cmd.CommandText = "INSERT INTO DeviceOAuth (DeviceID, OAuthTypeID, OAuthID, OAuthToken) VALUES (@DeviceID, @OAuthType, @OAuthID, @OAuthToken);";
                }
                else
                {
                    cmd.CommandText = "UPDATE DeviceOAuth SET OAuthID = @OAuthID, OAuthToken = @OAuthToken WHERE DeviceID = @DeviceID AND OAuthTypeID = @OAuthType;";
                }
